...
│   │   ├── pending_w2_task.json  ← tarea W2 registrada (handoff)
...
Usuario / Agente IA
   ...
   bago (CLI raíz)
        ↓
   bago_banner.py  ←── estado, inventario, task registrada, detector W9
        ↓
...
- Task W2 registrada si existe (⏳/✅)
- Tabla de comandos disponibles

### Política de alcance

No todos los comandos tienen semántica útil en `pack` y `repo`. BAGO los clasifica así:

| Alcance | Criterio |
|---|---|
| `pack-only` | Opera sobre `.bago`, estado, gobierno, workflows y validadores del sistema |
| `dual-scope` | Puede auditar el pack o el proyecto intervenido sin cambiar de verbo |
| `repo-only` | Opera explícitamente sobre el proyecto intervenido |

### Comandos pack-only de inspección y estado

| Comando | Herramienta | Descripción |
|---|---|---|
| `bago` | `bago_banner.py` | Banner completo con estado y comandos |
| `bago on` | `bago_on.py` | Activa explícitamente el modo BAGO sobre el host del pack y devuelve comandos/workflows |
...
| `bago task` | `show_task.py` | Muestra la tarea W2 registrada |
| `bago task --done` | `show_task.py` | Marca tarea como completada + registra en implemented_ideas.json |
| `bago task --clear` | `show_task.py` | Elimina la tarea registrada |
...
### Comandos pack-only de mantenimiento
...
| Comando | Herramienta | Descripción |
|---|---|---|
| `bago debug` | `bago_debug.py` | Detecta y repara bugs operativos y bugs silenciosos reparables del pack |
| `bago debug --repo [PATH]` | `bago_debug.py` | Detecta bugs del proyecto intervenido en el repo actual o en `PATH` |

### Comandos repo-only

| Comando | Herramienta | Descripción |
|---|---|---|
| `bago repo-on [PATH]` | `repo_on.py` | Activa un repo externo como objetivo operativo y devuelve comandos, workflows y cadenas detectadas |
| `bago repo-debug [PATH]` | `bago_debug.py` | Alias explícito de `bago debug --repo [PATH]` |
| `bago repo-lint [PATH]` | `repo_runner.py` | Ejecuta `lint` del proyecto intervenido con el runner detectado |
| `bago repo-test [PATH]` | `repo_runner.py` | Ejecuta `test` del proyecto intervenido con el runner detectado |
| `bago repo-build [PATH]` | `repo_runner.py` | Ejecuta `build` del proyecto intervenido con el runner detectado |
...
Handoff W2 registrado por `bago ideas --accept N`. Campos:
...
| `show_task.py` | `bago task` | Muestra/gestiona la tarea W2 registrada |
...
| Herramienta | Comando | Descripción |
|---|---|---|
| `bago_banner.py` | `bago` | Cartel principal con estado, inventario, task registrada y comandos |
| `bago_debug.py` | `bago debug` / `bago debug --repo [PATH]` | Depuración del pack o del proyecto intervenido |
...
```bash
# 1. ARRANQUE
python3 bago                # ver estado + task registrada
...
```bash
python3 bago task           # ver tarea registrada
...
| **Reserva** | Hallazgo que no invalida el sistema pero impide considerarlo completamente maduro |
