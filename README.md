...
![health](https://img.shields.io/badge/health-%F0%9F%9F%A2%20100%2F100-brightgreen) ![tests](https://img.shields.io/badge/tests-%E2%9C%85%20122%2F122-brightgreen) ![tools](https://img.shields.io/badge/tools-137-blue) ![chg](https://img.shields.io/badge/CHGs-113%20🏆-gold) ![version](https://img.shields.io/badge/versión-3.0-blue) ![langs](https://img.shields.io/badge/languages-py%20%7C%20js%20%7C%20go%20%7C%20rust-orange)

**`health 🟢 100/100`** · **`tests ✅ 122/122`** · **`tools 🔧 137`** · **`CHGs 🏆 113`** · **`versión 3.0`** · 10 workflows · Auto-gobernanza e inteligencia de intent integrada
...
> **`bago ideas`**: genera ideas **dinámicas** (state-aware: risk_matrix, debt_ledger, context_detector) + fallback garantizado. Puntúa por `priority` (0–100) · retorna **siempre 5–20 ideas** · historial en `IDEAS_BACKLOG.json`.
> Salida:
> ```
> Total ideas: 5 (contextuales=1, respaldo=4) [rango: 5–20]
> 1. [88] Limpiar riesgo residual: 3 hallazgos
>    siguiente paso: bago ideas --accept 1
> 2. [66] Alinear README con selector
> ...
> ```
...
├── .bago/
│   ├── tools/                    # 137 herramientas Python/JS (un archivo por comando)
...
Cada proyecto puede tener su propio estado BAGO independiente. BAGO usa variables de entorno para aislar el estado:

| Variable | Descripción |
|----------|-------------|
| `BAGO_STATE_DIR` | Directorio de estado del proyecto activo (`state/` por defecto) |
| `BAGO_PROJECT_ROOT` | Raíz del proyecto activo |
| `BAGO_HOME` | Directorio raíz del framework BAGO |

```bash
# Inicializar BAGO en un proyecto externo
bago init /ruta/a/mi-proyecto

# Ver proyectos vinculados y estado del framework
bago home

# Operar sobre un proyecto específico sin cambiar de directorio
BAGO_STATE_DIR=/mi-proyecto/.bago/state bago health
```

Cada proyecto inicializado con `bago init` recibe su propia estructura `.bago/state/` y su script `bago` local que apunta al framework central.
