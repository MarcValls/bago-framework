...
REGISTRY: dict[str, ToolEntry] = {
    ...
    "repo-clone": ToolEntry(
        ...
        preflight=[
            PreflightCheck("file", str(TOOLS_DIR / "repo_clone.py")),
        ],
    ),
    "repo-list": ToolEntry(
        ...
        preflight=[
            PreflightCheck("file", str(TOOLS_DIR / "repo_list.py")),
        ],
    ),
    "repo-switch": ToolEntry(
        ...
        preflight=[
            PreflightCheck("file", str(TOOLS_DIR / "repo_switch.py")),
        ],
    ),
    "code-quality": ToolEntry(
        cmd="code-quality", module="code_quality_orchestrator",
        description="ORQUESTADOR BAGO — Coordina agentes especializados (seguridad, lógica, smells, duplicaciones)",
        preflight=[
            PreflightCheck("file", str(TOOLS_DIR / "code_quality_orchestrator.py")),
            PreflightCheck("file", str(BAGO_ROOT.parent / "agents" / "security_analyzer.py")),
            PreflightCheck("file", str(BAGO_ROOT.parent / "agents" / "logic_checker.py")),
            PreflightCheck("file", str(BAGO_ROOT.parent / "agents" / "smell_detector.py")),
            PreflightCheck("file", str(BAGO_ROOT.parent / "agents" / "duplication_finder.py")),
        ],
    ),
    "db": ToolEntry(
        cmd="db", module="db_init",
        description="Gestión de la BD SQLite BAGO (init, status, ideas, reset-ideas)",
        preflight=[
            PreflightCheck("file", str(TOOLS_DIR / "db_init.py")),
        ],
    ),
    "select": ToolEntry(
        ...
        preflight=[
            PreflightCheck("file", str(TOOLS_DIR / "ideas_selector.py")),
        ],
    ),
    "start": ToolEntry(
        ...
        preflight=[
            PreflightCheck("file", str(TOOLS_DIR / "bago_start.py")),
        ],
    ),
}
...
def get_commands() -> dict[str, list[str]]:
    ...
    Format: {"cmd": [current_python, "/path/to/module.py"]}
    """
    return {
        name: [PYTHON, str(TOOLS_DIR / f"{entry.module}.py")]
