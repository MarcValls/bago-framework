...
REPO_CONTEXT = ROOT / "state" / "repo_context.json"
...
def auto_recommend() -> tuple[str, dict]:
    ...
    # Señal fuerte: clean start o repo nuevo/mismatch -> W1
    inventory = gs.get("inventory", {})
    if gs.get("history_migration_status") == "clean_start" and inventory.get("sessions", 0) == 0:
        ctx["is_new_repo"] = True

    if REPO_CONTEXT.exists():
        try:
            repo_ctx = json.loads(REPO_CONTEXT.read_text(encoding="utf-8"))
            if not repo_ctx.get("repo_root") or not repo_ctx.get("repo_fingerprint"):
                ctx["is_new_repo"] = True
        except Exception:
            ctx["is_new_repo"] = True
    else:
