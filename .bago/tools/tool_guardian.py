...
def _has_test_flag(filepath: Path) -> bool:
    """Detecta si el tool implementa '--test' en su __main__ block."""
    try:
...
def _is_in_integration(filepath: Path) -> bool:
    """Verifica si el tool tiene una entrada en integration_tests.py."""
    if not INTEG_FILE.exists():
        return False
    integ_text = INTEG_FILE.read_text("utf-8", errors="ignore")
    # Busca el nombre del archivo en _run("tool.py", ...) o en ALL_TESTS
    tool_name  = filepath.name  # e.g. "lint.py"
    return tool_name in integ_text


def _is_in_bago_script(filepath: Path) -> bool:
    """Verifica si el tool tiene routing en el bago script."""
    if not BAGO_SCRIPT.exists():
        return False
    script_text = BAGO_SCRIPT.read_text("utf-8", errors="ignore")
    # Convierte nombre de archivo a comando: lint.py → "lint", dead_code.py → "dead-code"
    cmd = filepath.stem.replace("_", "-")
    return f'"{cmd}"' in script_text or f"'{cmd}'" in script_text
...
def _self_test() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # T2 — tool con --test flag → no GUARD-E001
        t2 = root / "with_test.py"
        t2.write_text('"""Tool con test."""\nif __name__=="__main__":\n    if "--test" in sys.argv: pass\n')
        f2 = analyze([t2])
