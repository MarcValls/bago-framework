...
is_clean_start = (
    gs.get("history_migration_status") == "clean_start"
    and inventory.get("sessions") == 0
    and inventory.get("changes") == 0
    and inventory.get("evidences") == 0
    and real_sessions == 0
    and real_changes == 0
    and real_evidences == 0
)

last_completed = gs.get("last_completed_session_id")
if not last_completed:
    if not is_clean_start:
        errors.append("missing last_completed_session_id")
    else:
        for field in ("last_completed_task_type", "last_completed_workflow", "last_completed_change_id", "last_completed_evidence_id"):
            if gs.get(field) is not None:
                errors.append(f"{field} must be null in clean_start")
        if gs.get("last_completed_roles") not in ([], None):
            errors.append("last_completed_roles must be empty in clean_start")
else:
...
if not is_clean_start:
    required_files = [
        root / "state/changes/BAGO-CHG-023.json",
        root / "state/evidences/BAGO-EVD-023.json",
        root / "state/sessions/SES-CON-2026-04-11-001.json",
    ]
    for p in required_files:
        if not p.exists():
            errors.append(f"missing consolidation record: {p.relative_to(root).as_posix()}")
